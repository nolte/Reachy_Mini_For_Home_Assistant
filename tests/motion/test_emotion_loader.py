...
def test_load_one_shot_minimal(tmp_path):
    f = write_yaml(tmp_path, "wave.yaml", """\
    name: wave
...
def test_load_continuous_minimal(tmp_path):
    f = write_yaml(tmp_path, "hum.yaml", """\
    name: hum
...
def test_antenna_over_envelope_rejected(tmp_path):
    f = write_yaml(tmp_path, "danger.yaml", """\
    name: danger
    type: one_shot
    phases:
      - duration_s: 0.5
...
def test_envelope_boundary_value_accepted(tmp_path):
    ...
    f = write_yaml(tmp_path, "boundary.yaml", """\
    name: boundary
    type: one_shot
    phases:
      - duration_s: 0.5
...
def test_duplicate_emotion_name_raises(tmp_path):
    write_yaml(tmp_path, "first.yaml", """\
    name: same
    type: one_shot
    phases:
      - duration_s: 0.1
    ...
    """)
    write_yaml(tmp_path, "second.yaml", """\
    name: same
    type: continuous
    oscillators:
      - field: z_mm
...
def test_continuous_phase_offset_pi_is_antiphase(tmp_path):
    ...
    f = write_yaml(tmp_path, "anti.yaml", """\
    name: anti
    type: continuous
    oscillators:
